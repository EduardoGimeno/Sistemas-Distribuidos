...
defmodule Server do
    def listen_client do
        receive do
			...
			{c_pid, :fib_tr, interval, op} -> time1 = :os.system_time(:millisecond)
      						               	  fibonacci_list = Enum.map(interval, fn(x) -> Fib.fibonacci_tr(x) end)
      							              time2 = :os.system_time(:millisecond)
                                              send(c_pid, {:result, time2 - time1, fibonacci_list})
			{c_pid, :of, interval, op} -> time1 = :os.system_time(:millisecond)
      						              fibonacci_list = Enum.map(interval, fn(x) -> Fib.of(x) end)
      							          time2 = :os.system_time(:millisecond)
                                          send(c_pid, {:result, time2 - time1, fibonacci_list})
        end
        listen_client
